...
export const TxCoiainer = ({ index, control, register, remove }: TxCointerProps) => {
  ...
  const watchCoinTypeOut = useWatch({ name: `blocks.${index}.coinTypeOut`, control });
  ...
  useEffect(() => {
    ...
    preswap(watchCoinType, watchCoinTypeOut, amountIn)
      .then((res) => {
        const amountOut = decimalsToFloat(res?.estimatedAmountOut ?? 0, watchCoinTypeOut)
  ...
  return (
    <div className='flex items-center'>
      <div className='my-3 p-4 rounded-xl bg-gray-400'>
        ...
        <div className='flex items-center'>
          <input className='rounded-sm bg-gray-100 w-40 h-10 text-2xl p-2 mr-1'  {...register(`blocks.${index}.amount`)} type="number" step={calcStepFromCoinType(watchCoinType)} min="0" />
        ...
        {watchMethod === "swap" &&
          <div >
            ...
            <div className='flex items-center'>
              <input className='rounded-sm bg-gray-100 w-40 h-10 text-2xl p-2 mr-1' type="text" min="0" readOnly value={loading ? "..." : amountOut}
              />
              <select className='text-xl rounded-3xl w-20 h-10 bg-gray-100 text-center'

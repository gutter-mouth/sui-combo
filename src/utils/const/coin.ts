...
export const seacrhPool = (
  coinTypeA: (typeof constCoinTypes)[number],
  coinTypeB: (typeof constCoinTypes)[number],
) => {
  const poolAB = constPool.find(
    ({ coinTypeA: typeA, coinTypeB: typeB }) =>
      typeA === coinTypeA && typeB === coinTypeB,
  );
  if (poolAB) return { pool: poolAB, isAB: true };
  const poolBA = constPool.find(
    ({ coinTypeA: typeA, coinTypeB: typeB }) =>
      typeB === coinTypeA && typeA === coinTypeB,
  );
